...
// Two for the host pointers and rest fro specifying the Datasize

void kernel_cubic(int8_t **, int8_t **, int, int);

void kernel_cubic(int8_t **X, int8_t **Y, int nx, int ny)
{
...
vbx_byte_t *varb=vbx_sp_malloc( ny*sizeof(vbx_byte_t));
...
printf("Maximum Throughput on MXP for Poly3 (Cubic) in (Gops/sec) is -> %g Gops/sec\n",(nx*ny*ops)/(seconds*pow(10,9)));
